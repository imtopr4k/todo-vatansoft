...
import { DirectChatMessage } from '../models/DirectChat';
...
r.post('/send-message', requireAuth, async (req, res) => {
  try {
    ...
    // Agent bilgisini al
    const agent = await Agent.findById(auth.sub);
    if (!agent) {
...
// Chat geçmişini al
r.get('/chat-history/:ticketId', requireAuth, async (req, res) => {
  try {
...
r.post('/user-reply', async (req, res) => {
  try {
    ...
    const ticket = await Ticket.findOne({ 'telegram.from.id': Number(userId) })
      .sort({ createdAt: -1 });
  ...
  try {
    ...
    const usersWithMessages = await Promise.all(
      tickets.map(async (ticket) => {
        const lastMessage = await ChatMessage.findOne({ ticketId: ticket._id })
          .sort({ timestamp: -1 })
          .lean();
...
r.post('/mark-read/:ticketId', requireAuth, async (req, res) => {
  try {
    const { ticketId } = req.params;
    
    await ChatMessage.updateMany(
      { ticketId, fromType: 'user', read: false },
      { $set: { read: true } }
    );
...
r.get('/direct-users', requireAuth, async (req, res) => {
  try {
    // Tüm chatId'leri benzersiz şekilde al
    const userChats = await DirectChatMessage.aggregate([
      {
        $group: {
          _id: '$chatId',
          userId: { $first: '$userId' },
    ...
    const usersWithMessages = await Promise.all(
      userChats.map(async (chat) => {
        const lastMessage = await DirectChatMessage.findOne({ chatId: chat._id })
          .sort({ timestamp: -1 })
          .lean();

        const unreadCount = await DirectChatMessage.countDocuments({
          chatId: chat._id,
          fromType: 'user',
          read: false
        });
        ...
        const ticket = await Ticket.findOne({ 'telegram.from.id': chat.userId })
          .sort({ createdAt: -1 })
          .populate('assignedTo')
          .lean();
...
r.get('/direct-history/:chatId', requireAuth, async (req, res) => {
  try {
    const { chatId } = req.params;
    
    const messages = await DirectChatMessage.find({ chatId: Number(chatId) })
      .sort({ timestamp: 1 })
      .limit(200)
      .lean();
...
r.post('/direct-send', requireAuth, async (req, res) => {
  try {
    ...
    const lastUserMsg = await DirectChatMessage.findOne({ chatId: Number(chatId) }).sort({ timestamp: -1 });
    if (!lastUserMsg) {
...
r.post('/direct-mark-read/:chatId', requireAuth, async (req, res) => {
  try {
    const { chatId } = req.params;
    
    await DirectChatMessage.updateMany(
      { chatId: Number(chatId), fromType: 'user', read: false },
      { $set: { read: true } }
    );
