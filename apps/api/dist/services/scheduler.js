...
async function checkUnattendedTickets() {
    ...
    try {
        const now = new Date();
        // TEST: 30 saniye (sonra 5 dakikaya dönüştürün: 5 * 60 * 1000)
        const testTime = new Date(now.getTime() - 30 * 1000);
        ...
        // 3. createdAt 30 saniyeden eski - En az 30 saniye geçmiş
            ...
            try {
                ...
                for (const agent of activeAgents) {
                    if (agent.telegramUserId) {
                        try {
                            await (0, telegram_1.sendDM)(agent.telegramUserId, warningMessage);
                ...
                ticket.unattendedWarningAt = now;
                await ticket.save();
