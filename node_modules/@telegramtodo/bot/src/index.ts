...
import { createTicketAndAssign, markResolved, markUnreachable, registerUserChatId, saveUserReply } from './apiClient';

console.log('[bot] Imports loaded');
console.log('[bot] BOT_TOKEN:', env.BOT_TOKEN ? 'Available' : 'MISSING!');
...
function missingFields(p: Parsed): string[] {
  const miss: string[] = [];
  if (!p.id) miss.push('id |ID |Id|Uye No|uye no|');
